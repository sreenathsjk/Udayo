...
import { MessageSquare, ShieldCheck, X, ArrowUpRight, HelpCircle } from 'lucide-react';
...
export default function WhatsAppQuickAction({ userLocality = 'Maruthi Nagar' }: WhatsAppQuickActionProps) {
  ...
  const getLocalityDetail = (locality: string) => {
    if (locality === 'Ramachandra Nagar') {
      return {
        name: 'Saraswathi Garu',
        avatar: 'S',
        label: 'Verified Area Manager (Ramachandra Sector)'
      };
    }
    if (locality === 'Saptagiri Colony') {
      return {
        name: 'V. Krishna',
        avatar: 'K',
        label: 'Verified Area Manager (Saptagiri Sector)'
      };
    }
    return {
      name: 'Suresh K. Prasad',
      avatar: 'S',
      label: 'Verified Area Manager (Maruthi Sector)'
    };
  };

  const activeManager = getLocalityDetail(userLocality);

  // Dynamic preset templates list
  const getTemplates = (managerName: string): QuickTemplate[] => [
    {
      id: 'trial',
      badge: '🚀 Trial Box',
      title: 'Unlock 7-Day Free Trial',
      text: `Namaste Udayos Support! I am from Anantapur. I am interested in testing your daily home subscription on a 7-Day Free Trial. Please guide me on assigning ${managerName} or our regional manager.`
    },
    {
      id: 'water',
      badge: '💧 Water Swapping',
      title: 'Request 20L Water Can Refill',
      text: `Namaste ${managerName.split(' ')[0]}! Please schedule a 20L ISI bubbletop water can replacement today. Sourced can is placed outside the gate.`
    },
    {
      id: 'milk_extra',
      badge: '🥛 Milk Volume',
      title: 'Add 1 Extra Milk packet and Curd bundle',
      text: `Namaste ${managerName.split(' ')[0]}, please put 1 extra Nandini Blue packet and 1 local curd cup in tomorrow morning's basket.`
    },
    {
      id: 'pause',
  ...
  const templates = getTemplates(activeManager.name);
  ...
  const getActiveText = () => {
    ...
    const template = templates.find(item => item.id === selectedActionId);
        ...
        {showNotification && !isOpen && (
          ...
          >
            ...
            <span className="font-semibold block leading-normal">
              💬 <strong>{activeManager.name.split(' ')[0]} Manager is online!</strong> Click to chat on WhatsApp instantly.
            </span>
            <button 
              onClick={(e) => {
                e.stopPropagation();
        ...
        >
          ...
          <div className="bg-[#075E54] text-white p-4.5 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="relative">
                <div className="w-10 h-10 rounded-full bg-emerald-800 border border-white/20 flex items-center justify-center font-bold font-serif">{activeManager.avatar}</div>
                <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 border border-[#075E54] rounded-full"></div>
              </div>
              <div>
                <h4 className="font-bold text-xs flex items-center gap-1 leading-none text-white font-sans">
                  {activeManager.name}
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-300 fill-emerald-800" />
                </h4>
                <span className="text-[10px] text-white/50 block mt-0.5">{activeManager.label} • On-Duty 🟢</span>
              </div>
            </div>
            <button 
          ...
          <div className="bg-white p-3 border-t border-brand-stone space-y-3 shrink-0">
            ...
            <p className="text-[10px] text-center text-gray-400 italic">
              *Opens direct thread with your manager {activeManager.name}. Verify details free.
            </p>
          </div>
